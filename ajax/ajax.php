...
// Get All Table Names From the Database
    ...
    if(!empty($sqlScript))
    {
        ...
        //$backup_file_name = '../backup/'.date('YmdHis').'.sql';
...
if(isset($_POST['getNotify'])){
    ...
    $ct="SELECT * FROM message WHERE voka_id='$vokaId'";
    $ctrun=$conn->query($dbconnection,$ct);
    $actCount= $conn->sqlnum($ctrun);

    $stfmsg="SELECT * FROM message WHERE voka_id='$vokaId' ORDER BY date DESC LIMIT 10";
    ...
    $msgc="SELECT * FROM memo WHERE voka_id='$vokaId' AND status='Active' ORDER BY tstamp DESC LIMIT 20";
    $msgRun=$conn->query($dbconnection,$msgc);
    $memocount= $conn->sqlnum($msgRun);

    $memqry="SELECT * FROM memo WHERE voka_id='$vokaId' AND status='Active' ORDER BY tstamp DESC";
    $memqryrun=$conn->query($dbconnection,$memqry);
    $memomsg="";
    while($data=$conn->fetch($memqryrun)) {
        $usr = $data['usr'];
        $getimg = "SELECT img FROM staff_rec WHERE voka_id='$usr'";
    ...
    $get="SELECT message, caption, id FROM message WHERE voka_id='$vokaId' AND status='Active' ORDER BY date ASC LIMIT 1";
...
if(isset($_POST['apprformyear'])){
	...
	if($period=='mid_yr'){
		...
		$chktrun=$conn->query($dbconnection,$chkt);
		...
		$midrun=$conn->query($dbconnection,$mid);
...
if(isset($_POST['supappryear'])){
    ...
    $tgtrun=$conn->query($dbconnection,$tgtchk);
    ...
    }else
    {
        if ($period == 'mid_yr') {
            ...
            $chkendrun = $conn->query($dbconnection, $chkend);
...
if(isset($_POST['getList'])) {
    ...
    if($lvid=="dept"){
        ...
        $selcomrun=$conn->query($dbconnection,$selcom);
...
if(isset($_POST["type"])){
    ...
    $chequeAvailable = $conn->query($dbconnection,$sel);
...
if(isset($_POST['updatedob'])){
    ...
    if($conn->query($dbconnection,$upd)){
        print "OK";
    }
    else{
        print "NOT";
